...
def md_to_html_with_pandoc(md_path: Path, html_path: Path):
    """
    Converts a Markdown file to HTML using pandoc (pre-installed in GitHub Actions).
    """
    ...
    try:
        ...
        print(f"[OK] Converted {md_path} -> {html_path}")
    except FileNotFoundError:
        print("[WARN] pandoc not found! HTML not generated.")
...
def save_file(base_dir, repo, file_path, content_bytes):
    ...
    # If README.md, generate HTML using pandoc
    if rel_path.name.lower() == "readme.md":
        html_path = out_path.with_suffix(".html")
        with out_path.open("wb") as f:
            f.write(content_bytes)  # ensure the file exists for pandoc
        md_to_html_with_pandoc(out_path, html_path)
